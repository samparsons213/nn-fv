...
from keras.layers import LSTM, BatchNormalization, Activation, Bidirectional, Conv1D
...
reg_lambda=0.03
attention_reg_lambda=0.03
# Instantiate LSTM model for eye movement analysis
model = Sequential()
model.add(Conv1D(32, 5, activation='relu', input_shape=(max_length, data_dim), kernel_regularizer=regularizers.l2(reg_lambda)))
model.add(BatchNormalization())
model.add(Conv1D(32, 5, activation='relu', kernel_regularizer=regularizers.l2(reg_lambda)))
model.add(BatchNormalization())
model.add(Conv1D(64, 5, activation='relu', kernel_regularizer=regularizers.l2(reg_lambda)))
model.add(BatchNormalization())
model.add(AttentionWithContext(W_regularizer=regularizers.l2(attention_reg_lambda),
                               u_regularizer=regularizers.l2(attention_reg_lambda),
                               b_regularizer=regularizers.l2(attention_reg_lambda)))
...
if(detect_pca):
    ...
    model.compile(optimizer=sgd(lr=0.003, decay=1e-4, momentum=0.65),
                  loss='binary_crossentropy',
                  metrics=['accuracy'])
