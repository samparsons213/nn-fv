...
model = Sequential()
model.add(Conv1D(78, 9, activation='relu', input_shape=(max_length, data_dim), kernel_regularizer=regularizers.l2(0.1021)))
model.add(BatchNormalization())
model.add(Conv1D(64, 1, activation='relu', kernel_regularizer=regularizers.l2(0.1273)))
model.add(BatchNormalization())
model.add(Conv1D(32, 1, activation='sigmoid', kernel_regularizer=regularizers.l2(0.066)))
model.add(BatchNormalization())
model.add(AttentionWithContext(W_regularizer=regularizers.l2(0.8031),
                               u_regularizer=regularizers.l2(0.2929),
                               b_regularizer=regularizers.l2(0.2910)))
model.add(Dropout(0.0844))
...
if(detect_pca):
    ...
    model.compile(optimizer=sgd(lr=0.00407, decay=1e-4, momentum=0.69251),
                  loss='binary_crossentropy',
                  metrics=['accuracy'])
