...
model = Sequential()
model.add(LSTM(num_hidden, input_shape=(max_length, data_dim)))
...
if(detect_pca):
    ...
    model.compile(optimizer=adam(),
                  ...
                  metrics=['accuracy'])
    model.fit(training_data._input_data, to_categorical(training_data._output_data),
    ...
    model.fit(training_data._input_data, training_data._output_data,
              validation_data =[testing_data._input_data, testing_data._output_data],
              batch_size=16, epochs=epochs, callbacks=[checkpoint])


best_validation = load_model(sorted(glob.glob('./weights-eye*'))[-1])
score = model.evaluate(testing_data._input_data, testing_data._output_data, batch_size=16)
